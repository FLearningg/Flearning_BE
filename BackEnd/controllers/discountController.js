    ...
    try {
        ...
        if (!discountCode || !description || !category || !type || !value) {
            return res.status(400).json({
                success: false,
                message: "Vui lòng cung cấp đầy đủ thông tin bắt buộc (discountCode, description, category, type, value)"
            });
        }
...
exports.updateDiscount = async (req, res) => {
    try {
        ...
        // Validate value based on type
        if (updateData.type === "percent" && updateData.value && 
        ...
        if (updateData.type === "fixedAmount" && updateData.value && updateData.value < 0) {
            return res.status(400).json({
                success: false,
                message: "Giá trị giảm giá phải lớn hơn 0"
            });
...
exports.deleteDiscount = async (req, res) => {
    try {
        ...
        if (discount.usage > 0) {
            return res.status(400).json({
                success: false,
                message: "Không thể xóa discount đã được sử dụng. Hãy đặt trạng thái thành 'inActive' thay vì xóa."
            });
        }
